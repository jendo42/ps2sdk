...
// This is not an exaustive list of all commands being used. Some commands are generated by SECRMAN
// See usages of functions beginning with card_auth in SECRMAN.
static const u8 mcman_cmdtable[36] = {
	0x11, 0x04, // 00, Probe, mcman/mcman_cardchanged
	0x12, 0x04, // 01, ???, mcman/mcman_eraseblock
	0x21, 0x09, // 02, Start Erase, mcman/mcman_eraseblock
	0x22, 0x09, // 03, Start Write, mcman/McWritePage
	0x23, 0x09, // 04, Start Read, mcman/mcman_readpage
	0x24, 0x86, // 05, ???, ???
	0x25, 0x04, // 06, ???, ???
	0x26, 0x0d, // 07, Get Specs, mcman/McGetCardSpec
	0x27, 0x05, // 08, Set Terminator, mcman/mcman_probePS2Card
	0x28, 0x05, // 09, Get Terminator, mcman/mcman_probePS2Card2;mcman/mcman_probePS2Card
	0x42, 0x86, // 0A, Write Data, mcman/McWritePage
	0x43, 0x86, // 0B, Read Data, mcman/mcman_readpage
	0x81, 0x04, // 0C, Read/Write End, mcman/McWritePage;mcman/mcman_readpage
	0x82, 0x04, // 0D, Erase Block, mcman/mcman_eraseblock
	0x42, 0x06, // 0E, Write Data, mcman/McWritePage
	0x43, 0x06, // 0F, Read Data, mcman/mcman_readpage
	0xbf, 0x05, // 10, ???, ???
	0xf3, 0x05, // 11, ???, mcman/mcman_resetauth
};
...
void mcman_initPS2com(void)
{
	...
	DPRINTF("mcman_initPS2com registering secrman_mc_command callback\n");
...
int mcman_cardchanged(int port, int slot)
{
	...
	DPRINTF("mcman_cardchanged sio2cmd port%d slot%d\n", port, slot);

	sio2packet_add(port, slot, 0xffffffff, NULL);
	sio2packet_add(port, slot, 0x00, NULL);
	...
	if (retries >= 5) {
		DPRINTF("mcman_cardchanged sio2cmd card changed!\n");
...
#if !defined(BUILDING_XFROMMAN) && !defined(BUILDING_VMCMAN)
	sio2packet_add(port, slot, 0xffffffff, NULL);
	sio2packet_add(port, slot, 0x01, NULL);
	sio2packet_add(port, slot, 0xfffffffe, NULL);
...
int McWritePage(int port, int slot, int page, void *pagebuf, void *eccbuf) // Export #19
{
	...
	do {
		...
		if (((mcman_sio2packet.stat6c & 0xF000) != 0x1000) || (p[3] != 0x5a))
			continue;
#elif defined(BUILDING_VMCMAN)
		if (mcman_iomanx_backing_write(port, slot, page, pagebuf, eccbuf))
			continue;
#elif defined(BUILDING_XFROMMAN)
...
int mcman_resetauth(int port, int slot)
{
	...
	DPRINTF("mcman_resetauth sio2cmd port%d slot%d\n", port, slot);
	...
	if (retries >= 5) {
		DPRINTF("mcman_resetauth sio2cmd card changed!\n");
...
int mcman_probePS2Card2(int port, int slot)
{
	...
	DPRINTF("mcman_probePS2Card2 sio2cmd failed (mc detection failed)\n");
#else
...
int mcman_probePS1Card2(int port, int slot)
{
	...
	DPRINTF("mcman_probePS1Card2 port%d slot%d\n", port, slot);
	...
	do {
		...
		mcsio2_transfer(port, slot, &mcman_sio2packet_PS1PDA);
...
int mcman_probePS1Card(int port, int slot)
{
	...
	DPRINTF("mcman_probePS1Card port%d slot%d\n", port, slot);
...
int mcman_probePDACard(int port, int slot)
{
	...
	DPRINTF("mcman_probePDACard port%d slot%d\n", port, slot);
...
int McWritePS1PDACard(int port, int slot, int page, void *buf) // Export #30
{
	...
	//DPRINTF("McWritePS1PDACard port%d slot%d page %x\n", port, slot, page);
...
int McReadPS1PDACard(int port, int slot, int page, void *buf) // Export #29
{
	...
	//DPRINTF("McReadPS1PDACard port%d slot%d page %x\n", port, slot, page);
